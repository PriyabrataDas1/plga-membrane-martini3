...
np_gro = "systems/nanoparticle/plga_np_10nm_neg1.gro"
outdir = "systems/combined"
out_gro = f"{outdir}/plga_membrane_initial.gro"
...
for line in np_atoms:
    ...
    combined_atoms.append(
        f"{resnr:5d}{resname:<5}{atomname:>5}{atom_id:5d}{x:8.3f}{y:8.3f}{z:8.3f}"
    )
    atom_id += 1

with open(out_gro, "w") as f:
    f.write("PLGA nanoparticle above POPC/POPE/CHOL MARTINI membrane\n")
...
print(f"Nanoparticle beads: {np_n}")
print(f"Total beads: {len(combined_atoms)}")
print("NP shifted above membrane.")
